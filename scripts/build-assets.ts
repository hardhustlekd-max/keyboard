...
import { buildApkBuffer } from './build-apk';
...
`);
  ...
  // Generate real compiled Android APK package
  const apkBuffer = await buildApkBuffer();
  ...
  // Save true APK package
  fs.writeFileSync(path.join(wwwDir, 'AmharicKeyboard.apk'), apkBuffer);
  fs.writeFileSync(path.join(publicDir, 'AmharicKeyboard.apk'), apkBuffer);
  ...
  if (fs.existsSync(landingHtmlPath)) {
    ...
    if (fs.existsSync(distDir)) {
      ...
      fs.writeFileSync(path.join(distDir, 'AmharicKeyboard.apk'), apkBuffer);
      fs.writeFileSync(path.join(distDir, 'Amharic_Android_Keyboard_Project.zip'), zipBuffer);
      fs.writeFileSync(path.join(distWwwDir, 'AmharicKeyboard.apk'), apkBuffer);
